...
import com.bhardwaj.navigation.transform.RootTransformation
import kotlin.math.abs

class SlidingRootNavLayout(context: Context?) : FrameLayout(context!!), SlidingRootNav {
    private val flingMinVelocity: Float
    ...
    private fun changeMenuVisibility(animated: Boolean, newDragProgress: Float) {
        ...
        if (animated) {
            ...
            rootViews?.let { rootTransformation!!.transform(dragProgress, it) }
            requestLayout()
        }
    }
    ...
    private inner class ViewDragCallback : ViewDragHelper.Callback() {
        ...
        override fun onViewReleased(releasedChild: View, xvel: Float, yvel: Float) {
            val left = if (abs(xvel) < flingMinVelocity) positionHelper!!.getLeftToSettle(
                dragProgress,
                maxDragDistance
    ...
    init {
        ...
        flingMinVelocity = ViewConfiguration.get(context).scaledMinimumFlingVelocity.toFloat()
